...
To create a bounty for your contract, inherit from the base `Bounty` contract and provide an implementation for ```deployContract()``` returning the new contract address.::
	...
	import {Bounty, Target} from "./pegtoken/Bounty.sol";
...
For each researcher who wants to hack the contract and claims the reward, refer to our `Test <https://github.com/PegToken/PegToken/blob/master/test/Bounty.js/>`_ for the detail.
